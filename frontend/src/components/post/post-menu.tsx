...
import { MoreHorizontal, Pencil, Trash2, Flag, AlertTriangle, ShieldBan } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function PostMenu({ post, onDeleted }: PostMenuProps) {
  ...
  const handleWarnAuthor = () => {
    warnUser.mutate({ userId: post.author.id });
  };
  ...
  // Other user's post: report + admin actions
  return (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
        ...
        <DropdownMenuContent align="end">
          ...
          {isAdmin && (
            <>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={handleAdminRemove}>
                <Trash2 className="mr-2 size-4" />
                Xoa bai viet
              </DropdownMenuItem>
      ...
      <AlertDialog open={banConfirmOpen} onOpenChange={setBanConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cam tac gia?</AlertDialogTitle>
            <AlertDialogDescription>
