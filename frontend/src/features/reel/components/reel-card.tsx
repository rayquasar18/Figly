...
export function ReelCard({ reel, isActive, isMuted, onToggleMute, shouldLoad }: ReelCardProps) {
  ...
  const handleTap = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      setTimeout(() => {
        ...
        playPauseTimerRef.current = setTimeout(() => setShowPlayPause(false), 500);
      }, 300);
    },
  ...
  return (
    <div className="relative h-full w-full bg-black">
      ...
      {/* Actions (right side) */}
      <div className="z-20">
        <ReelActions reel={reel} onCommentClick={() => setCommentsOpen(true)} />
      </div>
      ...
      {/* Comments sheet */}
      <ReelCommentsSheet open={commentsOpen} onOpenChange={setCommentsOpen} postId={reel.id} />
    </div>
  );
}
