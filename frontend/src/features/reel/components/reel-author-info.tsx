  ...
  return (
    <div className="absolute bottom-16 left-3 max-w-[70%]">
      ...
      <div className="flex items-center gap-2">
        ...
        <Link href={`/${reel.author.username ?? ''}`} className="text-sm font-semibold text-white">
      ...
      {reel.caption && (
        <div className="mt-1.5">
          <p className={`text-sm text-white ${!expanded ? 'line-clamp-2' : ''}`}>{reel.caption}</p>
          {!expanded && reel.caption.length > 100 && (
            <button onClick={() => setExpanded(true)} className="text-sm text-white/70">
              ...xem them
            </button>
          )}
        </div>
