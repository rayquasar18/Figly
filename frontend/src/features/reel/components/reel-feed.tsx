...
export function ReelFeed({ reels, fetchNextPage, hasNextPage, isFetchingNextPage }: ReelFeedProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Space') {
        ...
        const container = containerRef.current;
        if (!container) return;
        const activeEl = container.querySelector(`[data-reel-index="${activeIndex}"]`);
  ...
  if (reels.length === 0) {
    return (
      <div className="flex h-[100dvh] flex-col items-center justify-center bg-black px-6 text-center">
        ...
        <h2 className="text-xl font-semibold text-white">Chua co reel nao</h2>
        <p className="mt-2 max-w-sm text-sm text-white/70">
          Hay la nguoi dau tien chia se reel! Quay video va chia se voi cong dong.
        </p>
        <Button onClick={openCreateReel} className="mt-6">
          Tao reel dau tien
        </Button>
      </div>
    );
